...
if __name__ == "__main__":
    ...
    if args.train:
        file_name = args.train.split("/")[-1].split(".")[0]
        write_csv(fasta_to_rows(args.train), out_dir / f"{file_name}.csv")
    if args.test:
        file_name = args.test.split("/")[-1].split(".")[0]
        write_csv(fasta_to_rows(args.test),  out_dir / f"{file_name}.csv")
